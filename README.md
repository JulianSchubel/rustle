...
Once complete generate a sample dataset as follows. Note that ndjson sample
data can be generated by providing the value `ndjson` to the `--format` option.
...
```bash
./rustle run sample.csv metrics.db --buffer-size 10000 --batch-size 1000
```
